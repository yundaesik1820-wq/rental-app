...
const STATUS_ICON = { 승인대기: "⏳", 승인됨: "✅", 교사서명대기: "✍️", 대여중: "🚀", 보류: null, 거절됨: "❌", 반납완료: null };
...
// ── 가로 카드 캐러셀 (좌우 스냅 스크롤) ────────────────────
// 최신 카드가 맨 왼쪽(경계). 왼쪽으론 안 넘어가고 오른쪽으로 넘겨야 이전 건이 보임.
function CardCarousel({ active, children }) {
  ...
  const measure = useCallback(() => {
    ...
    if (child) setH(prev => (prev === child.offsetHeight ? prev : child.offsetHeight));
  }, []);
  useEffect(() => {
    if (!active) return;
    measure();
  ...
  }, [active, children, measure]);
  if (!active) return <>{children}</>;
  return (
    <>
      <style>{`
        .hcards::-webkit-scrollbar { height:6px; }
        .hcards::-webkit-scrollbar-thumb { background:${C.border}; border-radius:999px; }
        .hcards > * {
          flex: 0 0 100%;
          max-width: 100%;
      ...
      `}</style>
      <div ref={ref} className="hcards" onScroll={measure} style={{
        display:"flex", overflowX:"auto", overflowY:"hidden",
        scrollSnapType:"x mandatory", scrollBehavior:"smooth", WebkitOverflowScrolling:"touch",
        paddingBottom:12, height: h || undefined, alignItems:"flex-start",
      }}>
        {children}
      </div>
    </>
...
function FacilityManager({ requests, subAdmin, isTeacher, isSuper, focusId, onConsumed }) {
  ...
  // 🔔 알림 딥링크 — 해당 시설 대여 건으로 이동 + 스크롤 + 하이라이트
  const [flashId, setFlashId] = useState(null);
  useEffect(() => {
    if (!focusId || !requests.length) return;
    const r = requests.find(x => x.id === focusId);
    if (r) { setTab(r.status); setFlashId(focusId); }
    onConsumed?.();
  }, [focusId, requests]);
  useEffect(() => {
    if (!flashId) return;
    const t1 = setTimeout(() => {
      document.getElementById(`facility-card-${flashId}`)?.scrollIntoView({ behavior: "smooth", block: "center" });
    }, 200);
...
<div style="text-align:center;margin-top:30px">
  ...
  return (
    <div>
      ...
      <CardCarousel active={filtered.length > 1}>
      {filtered.map(r => (
        <Card key={r.id} id={`facility-card-${r.id}`} style={{ marginBottom:12, border:`2px solid ${flashId===r.id ? C.teal : (statusColor[r.status]||C.border)+"30"}`, ...(flashId===r.id ? { boxShadow:`0 0 0 3px ${C.teal}66`, transform:"scale(1.01)" } : {}) }}>
...
export default function Rental({ subAdmin = false, focusId, facilityFocusId, onConsumed }) {
  ...
  // 🔔 알림 딥링크 — 시설 대여 건이면 시설 탭으로 전환 (포커스 처리는 FacilityManager가 담당)
  useEffect(() => { if (facilityFocusId) setMainTab("facility"); }, [facilityFocusId]);
...
<div style="text-align:center;margin-top:30px">
  ...
  return (
    <div>
      ...
      {mainTab === "facility" ? (
        <FacilityManager requests={facilityRequests} subAdmin={subAdmin} isTeacher={isTeacher} isSuper={isSuper} focusId={facilityFocusId} onConsumed={onConsumed} />
      ) : (
      <div>
      ...
      <CardCarousel active={sorted.length > 1}>
      {sorted.map(r => (
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "nowrap", minWidth: 0 }}>
                <span style={{ fontSize: 15, fontWeight: 800, color: C.navy, whiteSpace: "nowrap" }}>{r.studentName}</span>
                <span style={{ fontSize: 12, color: C.muted, whiteSpace: "nowrap" }}>{r.studentId ? r.studentId.slice(0,2)+"학번" : ""}</span>
                <span style={{ fontSize: 12, color: C.muted }}>·</span>
                <span style={{ fontSize: 12, color: C.muted, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.dept}</span>
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginLeft: 10, flexShrink: 0 }}>
              {r.status === "보류"
                ? <PauseCircle size={20} color={C.orange} />
                : STATUS_ICON[r.status] && <span style={{ fontSize: 18, display: "flex", alignItems: "center" }}>{STATUS_ICON[r.status]}</span>}
