...
export default function Reserve({ setTab }) {
  ...
  return (
    <div>
      ...
      <div style={{ background:`linear-gradient(135deg,#1B2B6B,#0D9488)`, borderRadius:16, padding:"14px 16px", marginBottom:16 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div style={{ position:"relative", background:"#fff", borderRadius:12, padding:"10px 14px", flex:1 }}>
            ...
            <div style={{ fontSize:12, fontWeight:700, color:"#1B2B6B", marginBottom:3 }}>여기는 전문가 예약 페이지예요!</div>
            <div style={{ fontSize:11, color:"#475569", lineHeight:1.5 }}>원하는 장비를 직접 골라서 신청할 수 있어요.<br/>장비 대여를 여기서 할 수 있어요 📦</div>
          </div>
        </div>
      </div>
      ...
      {/* 장바구니가 비었을 때 — 장비는 장비 목록에서 담는다 */}
      {cartTotal === 0 && (
        <div style={{ textAlign:"center", padding:"48px 20px" }}>
          <div style={{ fontSize:44, marginBottom:12 }}>🛒</div>
          <div style={{ fontSize:15, fontWeight:800, color:C.text, marginBottom:6 }}>담은 장비가 없어요</div>
          <div style={{ fontSize:13, color:C.muted, marginBottom:20, lineHeight:1.6 }}>
            장비 목록에서 필요한 장비를 담으면<br/>여기서 예약 신청을 할 수 있어요.
          </div>
          <Btn onClick={() => setTab && setTab("equip")} color={C.teal}>장비 담으러 가기</Btn>
        </div>
      )}
